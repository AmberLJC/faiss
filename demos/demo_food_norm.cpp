...
size_t q=1000;
    {
        ...
        size_t nt;
        float *xt = fvecs_read("food/food_learn_norm.txt", 2005 ,dim, &d, &nt);
...

    {
        ...
        size_t nb, d2;
        float *xb = fvecs_read("food/food_base_norm.txt",num,dim, &d2, &nb);
...
float *xq;

    {
        ...
        size_t d2;
        xq = fvecs_read("food/food_query_norm.txt",q,dim, &d2, &nq);
//	for(int i=0;i < 3000; i=i+1000){printf("%f , ",xq[i]);}
...

    {
        ...
        size_t nq2;
float *gt_int = fvecs_read("food/food_gt_norm.txt",q,100, &k, &nq2);
...
printf ("(****%.3f s****) SEARCHING  TIME. \n", elapsed()-t1 - t0);
        ...
        for(int i = 0; i < nq; i++) {
            int gt_nn = gt[i * k];
	    for(int j = 0; j < k; j++) {
  //if(i<2)         printf(" the %d th predicted 1-nn is pt %d\n",i,I[i*k+j]);
		if (I[i * k + j] == gt_nn ){ 
                    if(j < 1) n_1++;
                    if(j < 10) n_10++;
                    if(j < 100) n_100++;
