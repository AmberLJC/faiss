...
int main()
{
  ...
  // const char *index_key = "LSH128";
    // const char *index_key = "HNSW128_2x32";
    // const char *index_key = "Flat";
    // const char *index_key = "HNSW256";
...
size_t q=100;
    {
        ...
        size_t nt;
        float *xt = fvecs_read("dense_test/dense_test_learn.txt",  10000 ,dim, &d, &nt);
 //       for(int i=0; i < 100 ; i++){printf("%f , ",xt[i]);}
...

    {
        ...
        size_t nb, d2;
        float *xb = fvecs_read("dense_test/dense_test_base.txt",num,dim, &d2, &nb);
   //     for(int i=0; i < 100 ; i++){printf("%f , ",xb[i]);}
...
float *xq;

    {
        ...
        size_t d2;
        xq = fvecs_read("dense_test/dense_test_query.txt",q,dim, &d2, &nq);
...

    {
        ...
        size_t nq2;
float *gt_int = fvecs_read("dense_test/dense_test_gt.txt",q,100, &k, &nq2);
...
//printf("read %d ground truth with dimension: %d \n ",nq2,k);
 	...
 	//for(int i=0; i < 100 ; i++){printf("%d , ",gt[i]);}
 	delete [] gt_int;
    }
  ...
  //              printf(" the %d th predicted 1-nn is pt %d\n",i,I[i*k+j]);
		if (I[i * k + j] == gt_nn || 
			I[i * k + j]==gt[i * k+1] ||
 I[i * k + j]==gt[i * k+2] ||
		 I[i * k + j]==gt[i * k+3] || I[i * k + j]==gt[i * k+4]		) {
                    if(j < 1) n_1++;
                    if(j < 10) n_10++;
                    if(j < 100) n_100++;
                }
