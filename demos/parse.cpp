...
int main()
{
...

    {
        ...
        size_t nt;
        float *xt = fvecs_read("img_sift_learn_idx.txt", &d, &nt);
for(int i=0; i < 100 ; i++){printf("%f , ",xt[i]);}
...

    {
        ...
        size_t nb, d2;
        float *xb = fvecs_read("img_sift_base_idx.txt", &d2, &nb);
...
for(int i=128; i < 256 ; i++){printf("%f , ",xb[i]);}
...

    {
        ...
        size_t d2;
        xq = fvecs_read("img_sift_query_idx.txt", &d2, &nq);
...

    {
        ...
        size_t nq2;
        int *gt_int = ivecs_read("img_sift_gt_idx.txt", &k, &nq2);
